...
# Explicit Outfit Rules (User-defined pairings)
EXPLICIT_OUTFIT_RULES = {
    "1e346d74-5913-46f6-8774-a8f905a1dc38": [
        "bb06e7a6-85ae-44d3-bdc9-cb395a7e520c",
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706"
    ],
    "3b30b0c5-4cc3-4cf2-8efc-e9ea725cebf6": [
        "4ba0294d-ab7e-4806-8cb7-4787eb23e712",
        "bb06e7a6-85ae-44d3-bdc9-cb395a7e520c",
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706"
    ],
    "4ba0294d-ab7e-4806-8cb7-4787eb23e712": [
    ...
    "8e8ba0bc-75fc-4b75-b922-bfa3a43096b8": [
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706",
        "56a6f879-a788-43ee-bf16-7cfe924bee06"
    ],
    "8f237eb5-72ab-4341-a49e-2a5f5c5db03d": [
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706",
        "56a6f879-a788-43ee-bf16-7cfe924bee06"
    ],
    "56a6f879-a788-43ee-bf16-7cfe924bee06": [
    ...
    "64ff0526-8908-4d1a-8df2-249d0a415806": [
        "1e346d74-5913-46f6-8774-a8f905a1dc38"
    ],
    "a0468aba-b0b4-4ec0-9235-faa80994bed5": [
        "d3bd0855-be52-4de6-bc01-00d0f37df170",
        "bb06e7a6-85ae-44d3-bdc9-cb395a7e520c",
        "f7f3b260-320a-4072-ae8f-69ec12fdeb33"
    ],
    "bb06e7a6-85ae-44d3-bdc9-cb395a7e520c": [
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706",
        "f7f3b260-320a-4072-ae8f-69ec12fdeb33",
    ...
    "d3bd0855-be52-4de6-bc01-00d0f37df170": [
        "fc84e773-6043-4168-a312-d6ae3c1d8c2f",  # or a0468aba-b0b4-4ec0-9235-faa80994bed5
        "bb06e7a6-85ae-44d3-bdc9-cb395a7e520c",
        "f7f3b260-320a-4072-ae8f-69ec12fdeb33"
    ],
    "f6b654e4-7a92-4616-b83a-c0f2406e2a20": [
        "fd7d254e-a5f0-450c-9a94-ec3fd9a4a706"
    ]
}
...
def _create_explicit_outfit(seed_item_id: str, wardrobe_df: pd.DataFrame) -> dict:
    ...
    try:
        # Get the seed item by filename (extract from image_url)
        seed_item = None
        for _, item in wardrobe_df.iterrows():
            if item['image_url'] and seed_item_id in item['image_url']:
                seed_item = item
                break
        ...
        # Find the paired items in the wardrobe by filename
        paired_items = []
        for item_id in paired_item_ids:
            for _, item in wardrobe_df.iterrows():
                if item['image_url'] and item_id in item['image_url']:
...
def generate_stylist_recommendation():
    ...
    try:
        ...
        try:
            ...
            # Check for explicit outfit rules first
            outfits = []
            if seed_item_ids and len(seed_item_ids) == 1:
                # Single seed item - check for explicit rules
                seed_id = seed_item_ids[0]
                explicit_outfit = _create_explicit_outfit(seed_id, user_wardrobe_df)
                if explicit_outfit:
                    outfits = [explicit_outfit]
                    logger.info(f"✅ Using explicit outfit rule for seed item {seed_id}")
                else:
                    logger.info(f"ℹ️ No explicit rule found for {seed_id}, using AI recommendations")
            
            # If no explicit outfit found, use AI recommendations
            if not outfits:
                logger.info("🎯 Generating AI outfit recommendations...")
                outfits = recommender.recommend_outfits(
                    seed_item_ids=user_wardrobe_df['id'].tolist()[:5] if not seed_item_ids else seed_item_ids,
                    occasion=occasion,
                    num_outfits=num_outfits
                )
                logger.info(f"✅ Generated {len(outfits)} AI outfits")
            
            if outfits:
                # Convert outfits to the expected format
                formatted_outfits = []
                ...
                result = {
                    ...
                    'total_outfits': len(formatted_outfits),
                ...
                logger.info("✅ Successfully generated outfit recommendations using AI")
            else:
