...
import { detectCombination, createHardcodedOutfit, type HardcodedOutfit } from "@/utils/combinationUtils";
...
const OutfitRecommendations = () => {
  ...
  const generateOutfits = async () => {
    ...
    // Check for hardcoded combinations first
    console.log('Selected items for combination detection:', selected);
    const detectedCombination = detectCombination(selected);
    
    if (detectedCombination) {
      console.log(`Detected hardcoded combination: ${detectedCombination}`);
      
      toast({
        title: "Perfect combination found! ✨",
        description: `Loading your curated look...`
      });

      // Create hardcoded outfit
      const hardcodedOutfit = createHardcodedOutfit(detectedCombination);
      console.log('Hardcoded outfit created:', hardcodedOutfit);
      
      // Convert to frontend format
      const frontendOutfit = {
        id: hardcodedOutfit.id,
        title: hardcodedOutfit.title,
        caption: hardcodedOutfit.caption,
        items: hardcodedOutfit.items,
        type: 'hardcoded-combination',
        score: hardcodedOutfit.score,
        outfitImage: null
      };

      console.log('Frontend outfit created:', frontendOutfit);
      setGeneratedOutfits([frontendOutfit]);
      
      toast({
        title: "Curated look ready! 🎨",
        description: "Your perfect look has been loaded"
      });
    ...
    // Fallback to original AI logic if no hardcoded combination detected
    toast({
    ...
    try {
      // Try to use backend AI recommendations
          ...
          return {
            ...
            items: outfit.items.map(aiItem => {
              ...
              if (aiItem.image_url) {
                ...
                if (aiItem.image_url.startsWith('http://localhost:5000')) {
                  // Already has full URL
                  imageUrl = aiItem.image_url;
                } else if (aiItem.image_url.startsWith('/uploads/')) {
                  // Direct uploads path
              ...
              console.log(`Item ${aiItem.id} full data:`, aiItem);
              
              return {
                id: aiItem.id,
                name: aiItem.description || aiItem.name || `${aiItem.category} item`,
                image: imageUrl,
            ...
            type: 'ai-generated',
            score: outfit.score,
            outfitImage: null // We'll use individual item images instead
          };
        });

        console.log('Converted AI Outfits:', aiOutfits);
        console.log('First outfit items:', aiOutfits[0]?.items);
        console.log('First item image URL:', aiOutfits[0]?.items[0]?.image);
  ...
  return (
    <div className="min-h-screen bg-myntra-light-gray pt-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Generated Outfit - Single Premium Display */}
        {generatedOutfits.length > 0 && (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-bold text-pink-500">Your Curated Look ✨</h2>
              <Button 
                variant="outline" 
                onClick={handleGenerateClick}
                className="border-pink-500 text-pink-500 hover:bg-pink-500 hover:text-white px-6 py-2 rounded-full font-semibold transition-all duration-200 hover:shadow-lg"
              >
                Generate New Look
              </Button>
            </div>

            {/* Single Premium Outfit Display */}
            <div className="bg-gradient-to-br from-white to-pink-50 rounded-2xl shadow-2xl border border-pink-200 p-8">
              <div className="text-center mb-6">
                <div className="inline-flex items-center gap-2 bg-pink-500/20 text-pink-500 px-4 py-2 rounded-full text-sm font-semibold mb-4">
                  <Sparkles className="h-4 w-4" />
                  Curated by AI Stylist
                </div>
                <h3 className="text-2xl font-bold text-pink-500 mb-2">Perfect Match Found!</h3>
              </div>
              
              <div className="max-w-4xl mx-auto">
                {generatedOutfits
                  .filter(outfit => outfit.type === 'ai-generated' || outfit.type === 'hardcoded-combination')
                  .map((outfit) => (
                    <OutfitCard
                      key={outfit.id}
                      outfitId={outfit.id}
